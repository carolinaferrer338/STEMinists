...
print(sum([1/x for x in polsby_popper(CON_Part).values()])/9)
...
cd_dict =  recursive_tree_part(graph,range(9),ideal_population,'TOTPOP', epsilon = 0.02)
...
def pp_constraint(partition): 
    ...
    return sum([1/x for x in polsby_popper(partition).values()])/9 > 4
...
def run_markov_chain(proposal_function, constraint_choices, file_name, accept_function=accept.always_accept, seed=new_starting_seed, num_steps=100_000):
    ...
    for part in second_recom_chain:
        ...
        pps.append(sum([1/x for x in polsby_popper(part).values()])/9)
...
run_markov_chain(county_proposal, [pp_constraint, county_constraint, competitiveness_constraint], "Output/Testing_Convergence_1/Testing_01", num_steps=100_000)
print("First chain done")
run_markov_chain(county_proposal, [pp_constraint, county_constraint, competitiveness_constraint], "Output/Testing_Convergence_2/Testing_02", num_steps=100_000)
