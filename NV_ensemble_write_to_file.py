...
graph = Graph.from_json("state_data/nv/nv.json")
df = gpd.read_file("state_data/nv/nv.shp")
n = 4
...
Path("Output/NV_Testing_01/").mkdir(parents=True, exist_ok=True)

Path("Output/NV_Testing_02/").mkdir(parents=True, exist_ok=True)
...
run_markov_chain(first_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/NV_Testing_01/throwaway1", combined_acceptance, num_steps=100)
print("First chain done at", datetime.fromtimestamp(time.time()))
run_markov_chain(second_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/NV_Testing_02/throwaway2", combined_acceptance, num_steps=100)
