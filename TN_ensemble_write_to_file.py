...
graph = Graph.from_json("state_data/tn/tn.json")
df = gpd.read_file("state_data/tn/tn.shp")
n = 9
...
def county_splits(partition, df=df):
    ...
    counties = sum(df.groupby("COUNTYFP18")['current'].nunique()>1)
    return counties
...
def accept_higher_pp(partition):
    ...
    avg_pp_new = sum([1/x for x in polsby_popper(partition).values()])/n
    avg_pp_old = sum([1/x for x in polsby_popper(partition.parent).values()])/n
...
Path("Output/TN_Testing_01/").mkdir(parents=True, exist_ok=True)

Path("Output/TN_Testing_02/").mkdir(parents=True, exist_ok=True)

#markov chain definition and calling it to run
#also writes stuff to file
def run_markov_chain(seed, proposal_function, constraint_choices, file_name, accept_function, num_steps):
    ...
    for part in second_recom_chain:
        ...
        if temp %10 == 0:
            ...
            ndf.to_csv(f"./{file_name}_chain_outputs_{temp}.csv")

            mmd.to_csv(f"./{file_name}_mmd_outputs_{temp}.csv")
...
run_markov_chain(first_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/TN_Testing_01/throwaway1", combined_acceptance, num_steps=100)
print("First chain done at", datetime.fromtimestamp(time.time()))
run_markov_chain(second_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/TN_Testing_02/throwaway2", combined_acceptance, num_steps=100)
