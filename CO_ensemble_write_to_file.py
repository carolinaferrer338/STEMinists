...
graph = Graph.from_json("state_data/co/co.json")
df = gpd.read_file("state_data/co/co.shp")
n = 8
...
Path("Output/CO_Testing_01/").mkdir(parents=True, exist_ok=True)

Path("Output/CO_Testing_02/").mkdir(parents=True, exist_ok=True)
...
def run_markov_chain(seed, proposal_function, constraint_choices, file_name, accept_function, num_steps=100_000):
    ...
    for part in second_recom_chain:
        ...
        if temp %10_000 == 0:
            ...
            print("Step Number", temp, datetime.fromtimestamp(time.time()))
...
run_markov_chain(first_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/CO_Testing_01/Testing_01", combined_acceptance, num_steps=100)
print("First chain done at", datetime.fromtimestamp(time.time()))
run_markov_chain(second_seed, county_proposal, [ces_constraint, competitiveness_constraint, county_constraint], "Output/CO_Testing_02/Testing_02", combined_acceptance, num_steps=100)
